...
function forbidden(message = 'This account is not a member of a shop.') {
...
export async function resolveTenant(db, principal, env, now = () => new Date().toISOString()) {
  ...
  try {
    await db.batch([
      ...
      db.prepare('INSERT INTO households (id,name,created_at) VALUES (?,?,?)').bind(householdId, 'My shop', stamp),
    ...
    if (claimed && /unique|constraint|primary key/i.test(String(error?.message || ''))) throw forbidden('Shop setup is already in progress.');
    throw error;
  }
  const claimed = await db.prepare('SELECT household_id,owner_user_id FROM tenant_bootstrap WHERE singleton=1').first();
  if (claimed?.household_id !== householdId || claimed.owner_user_id !== userId) throw forbidden('Shop setup is already in progress.');
