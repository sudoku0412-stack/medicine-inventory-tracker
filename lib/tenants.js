...
export async function resolveTenant(db, principal, env, now = () => new Date().toISOString()) {
  ...
  try {
    await db.batch([
      ...
      db.prepare('INSERT OR IGNORE INTO household_settings (household_id,display_name,household_name,default_storage_location,updated_at) SELECT ?,display_name,household_name,default_storage_location,? FROM profile_settings WHERE singleton=1').bind(householdId, stamp)
    ]);
  } catch (error) {
